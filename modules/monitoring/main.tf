resource "google_logging_metric" "job_failure" {
  ...
  name        = "error_metric_for_${var.dbt_job_name}"
  description = "Cloud Run Job（${var.dbt_job_name}）の実行失敗を検知するログベースの指標"
...
resource "google_monitoring_alert_policy" "job_failure_alert" {
  project               = var.project_id
  display_name          = "Error Alert for ${var.dbt_job_name}"
  combiner              = "OR"
  severity              = "ERROR"
  enabled               = true
  ...
  conditions {
    display_name = "Error Condition for ${var.dbt_job_name}"
